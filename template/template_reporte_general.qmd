...
title: "IHEA 2026"
subtitle: |
  Reporte Encuesta de Intereses, Hábitos y Expectativas Académicas <br> **Todas las Carreras**
author: ""
format:
  revealjs:
...
if (is.data.frame(dat)) {
  ...
  tipo_admision_tbl <- readRDS(here("outputs", "cobertura_tipo_admision.rds"))
} else if (is.list(dat) && ("ihea" %in% names(dat))) {
  # Formato nuevo: lista con ihea + cobertura + tipo_admision
  ihea_full <- dat$ihea
  cobertura_tbl <- dat$cobertura
  tipo_admision_tbl <- if ("tipo_admision" %in% names(dat)) dat$tipo_admision else NULL
} else {
  stop("data_reproduction.rds tiene un formato inesperado. Debe ser data.frame o lista con $ihea.")
}

# IMPORTANTE: No filtrar por carrera - usar todos los datos
df <- ihea_full %>%
  filter(!is.na(carrera))

n_est <- nrow(df)
n_carreras <- n_distinct(df$carrera)

# Cobertura general
cobertura_total <- cobertura_tbl %>%
  summarise(
    n_matriculados = sum(n_matriculados, na.rm = TRUE),
    n_respondieron = sum(n_respondieron, na.rm = TRUE)
  ) %>%
  mutate(cobertura = n_respondieron / n_matriculados)

# Preparar datos de tipo de admisión para visualización
if (!is.null(tipo_admision_tbl)) {
  tipo_adm_viz <- tipo_admision_tbl %>%
    filter(tipo_admision != "Sin información") %>%
...
## Reporte General IHEA 2026 {canvas-title="true"}

::: {.columns}
::: {.column width="60%"}
### Resumen Global
* **Total Estudiantes:** `r format(n_est, big.mark = ",")` (Diurno Presencial)
* **Carreras Analizadas:** `r n_carreras`
* **Cobertura Institucional:** `r percent(cobertura_total$cobertura, accuracy = 0.1)` 
* **Matrícula Total:** `r format(cobertura_total$n_matriculados, big.mark = ",")`

```{r, echo=FALSE, results='asis'}
if (!is.null(tipo_adm_viz) && nrow(tipo_adm_viz) > 0) {
  # Filtrar solo Centralizada y Directa (excluir "Otra" y "Sin información")
  tipo_principal <- tipo_adm_viz %>% 
    filter(tipo_admision %in% c("Centralizada", "Directa"))
  
  if (nrow(tipo_principal) > 0) {
    for (i in 1:nrow(tipo_principal)) {
      cat(sprintf(
        "  - *%s:* %s (%s)\n",
        tipo_principal$tipo_admision[i],
        format(tipo_principal$n_matriculados[i], big.mark = ","),
        scales::percent(tipo_principal$pct_matriculados[i], accuracy = 0.1)
      ))
    }
  }
}
...
::: {.callout-note}
## Alcance
Este reporte presenta un análisis integrado de todas las carreras incluidas en la aplicación del instrumento IHEA 2026.
:::
:::
:::

## Composición por Tipo de Admisión

::: {.columns}
::: {.column width="50%"}
```{r, echo=FALSE, fig.width=14, fig.height=11}
if (!is.null(tipo_adm_viz) && nrow(tipo_adm_viz) > 0) {
  ggplot(tipo_adm_viz, aes(x = n_matriculados, y = reorder(tipo_admision, n_matriculados))) +
    geom_col(fill = brand_teal, width = 0.7) +
    geom_text(
      aes(label = paste0(scales::comma(n_matriculados), "\n(", scales::percent(pct_matriculados, accuracy = 0.1), ")")),
      hjust = -0.1, size = 8, fontface = "bold", color = brand_dark
    ) +
    scale_x_continuous(labels = scales::comma, expand = expansion(mult = c(0, 0.25))) +
    labs(
      title = "Matrícula por Tipo de Admisión",
  ...
  plot.new()
  text(0.5, 0.5, "Datos no disponibles", cex = 2)
}
```
:::
::: {.column width="50%"}
```{r, echo=FALSE, fig.width=14, fig.height=11}
if (!is.null(tipo_adm_viz) && nrow(tipo_adm_viz) > 0) {
  ggplot(tipo_adm_viz, aes(x = cobertura, y = reorder(tipo_admision, cobertura))) +
    geom_col(fill = brand_dark, width = 0.7) +
    geom_text(
      aes(label = paste0(scales::percent(cobertura, accuracy = 0.1), "\n(", n_respondieron, "/", n_matriculados, ")")),
      hjust = -0.1, size = 8, fontface = "bold", color = brand_dark
    ) +
    scale_x_continuous(labels = scales::percent, limits = c(0, max(tipo_adm_viz$cobertura, na.rm = TRUE) * 1.3)) +
    labs(
      title = "Cobertura por Tipo de Admisión",
      x = "Tasa de Respuesta",
      y = NULL
    ) +
    theme_minimal(base_size = 30) +
...
ggplot(df_p19, aes(x = n, y = reorder(item, n))) +
  ...
  labs(
    title = "Ranking de Necesidades Formativas (Global)",
    x = "Menciones Totales",
    y = NULL
  ) +
